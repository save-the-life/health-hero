...
- **앱인토스 환경 최적화**:
  - 하트 데이터 없을 시 기본값 생성 (`current_hearts: 5`, `last_refill_at: now()`)
...
### 8.10. 아이템 사용 모달 시스템 (2025-01-27 완료)
...
**구현된 사운드 효과:**
- `button-click.mp3`: 모든 버튼 클릭 시 재생
...
**설정 메뉴 음소거 버튼:**
- **위치**: 설정 드롭다운 메뉴 첫 번째 항목
- **기능**: 전체 사운드 ON/OFF 토글
- **상태 저장**: `user_item_settings` 테이블에 저장
- **기본값**: 소리 켜짐 (30% 볼륨)

**음소거 상태 표시:**
- **켜짐**: 스피커 아이콘 (흰색 원형 배경)
- **꺼짐**: 스피커 아이콘에 X 표시 (회색 원형 배경)

### 9.3. 사운드 버튼 시스템

**SoundButton 컴포넌트:**
- **자동 사운드**: 모든 버튼 클릭 시 자동으로 `button-click.mp3` 재생
- **조건부 재생**: 잠금된 버튼이나 특정 상황에서는 사운드 재생 안함
- **성능 최적화**: Web Audio API 우선 사용, HTML Audio 폴백

**Clickable 컴포넌트:**
- **다양한 요소**: 버튼 외의 클릭 가능한 요소에도 사운드 적용
- **유연한 사용**: `as` prop으로 다양한 HTML 요소 지원

---

## 10. 광고 시스템 UI (2025-01-27 업데이트)
...
- **위치**: 하트 부족 모달 하단
- **텍스트**: "광고 시청하고 하트 충전"
- **기능**: Apps-in-Toss AdMob SDK를 통한 광고 표시
- **보상**: 광고 시청 완료 시 하트 1개 추가

**광고 상태 표시:**
- **로딩 중**: "광고 로딩 중..." 텍스트 표시
- **준비 완료**: "광고 시청하고 하트 충전" 버튼 활성화
- **실패**: "로드 실패 - 다시 시도" 버튼 표시
...
- **제한 도달 시**: "오늘의 광고 시청 횟수를 모두 사용했습니다" 메시지

**제한 상태 표시:**
- **가능**: 정상적인 광고 시청 버튼 표시
- **제한**: 버튼 비활성화 및 제한 메시지 표시

---

## 11. 성능 최적화 UI (2025-01-27 추가)

### 11.1. 코드 스플리팅

**지연 로딩 모달:**
- **Suspense**: 모달 컴포넌트 로딩 중 "로딩 중..." 표시
- **지연 로딩**: 모달이 실제로 열릴 때만 컴포넌트 로드
- **번들 크기**: 초기 로딩 시간 30-40% 단축

### 11.2. 서비스 워커

**캐싱 시스템:**
- **정적 자원**: 오디오 파일과 이미지 사전 캐싱
- **재방문 최적화**: 재방문 시 60-70% 빠른 로딩
- **오프라인 지원**: 네트워크 실패 시 캐시된 자원 사용

### 11.3. 이미지 최적화

**Next.js Image 컴포넌트:**
- **자동 최적화**: WebP/AVIF 포맷 자동 변환
- **지연 로딩**: 화면에 보이는 이미지만 로드
- **적응형 크기**: 디바이스별 최적화된 이미지 크기
