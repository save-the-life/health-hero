...
- **일일 최대 100회** 광고 시청 가능 (2025-10-29 업데이트: 5회 → 100회)
- 자정에 광고 시청 횟수가 리셋됩니다
- 광고 시청 간격은 최소 3초입니다
...
- 일일 100회 제한 확인 (2025-10-29 업데이트: 5회 → 100회)
- 광고 시청 간격 확인 (최소 3초)
...
## TDS 모달 통합 (2025-10-29 업데이트)

### 1. 광고 결과 모달

- **기존**: OS 기본 `alert()` 사용
- **개선**: TDS Mobile `AlertDialog` 사용
- **컴포넌트**: `AdRewardDialog.tsx`
- **장점**: 
  - 일관된 토스 디자인 시스템
  - 커스터마이징 가능한 UI
  - 더 나은 사용자 경험
...
- **문제**: 전체 앱에 TDS Provider 적용 시 CSS 충돌
- **해결**: `TDSProvider` 컴포넌트로 격리
- **적용 범위**: `AdRewardDialog` 컴포넌트만

### 3. 광고 시청 횟수 제한 변경

**업데이트 내역** (2025-10-29):
- 일일 제한: **5회 → 100회**
- 적용 파일: 6개 SQL 파일 (모든 Supabase 설정)
- 목적: 테스트 및 실사용 편의성 향상

**Supabase 업데이트 필요**:
```sql
-- add_heart_by_ad 함수 업데이트
-- supabase/add-heart-by-ad.sql 파일 전체 실행
```
