...
const generateSQL = (data) => {
  ...
  const insertValues = data.map(row => {
    ...
    return `(${row.qnum}, '${escapeSql(row.topic)}', '${escapeSql(row.prompt)}', '[${choices.join(', ')}]', ${row.answer_index}, '${escapeSql(row.hint || '')}', '${escapeSql(row.explanation || '')}', '${escapeSql(row.difficulty_label || '보통')}', ${difficultyLevel})`;
...
const outputPath = path.join(__dirname, '..', 'supabase', 'update-quiz-data-with-hints-fixed.sql');
