...
export default function Phase3Page() {
  ...
  const isStageLocked = (stageNumber: number) => {
    // 현재 페이즈가 3보다 낮으면 모든 스테이지 잠금
    if (currentPhase < 3) {
      return true;
    }

    // 현재 페이즈가 3이고, 현재 스테이지보다 높은 스테이지는 잠금
    if (currentPhase === 3) {
      return stageNumber > currentStage;
    }

    // 현재 페이즈가 3보다 높으면 모든 스테이지 열림
    return false;
  };
  ...
  const handleStageClick = (stageNumber: number) => {
    ...
    router.push(`/game/quiz?phase=3&stage=${stageNumber}`);
  };
    ...
    return (
      <div className="relative h-screen overflow-hidden" style={{ height: '100vh', overflow: 'hidden' }}>
        ...
        <div className="absolute inset-0 z-0">
          <SafeImage
            src="/images/backgrounds/background-phase3.png"
            alt="페이즈 3 배경"
            fill
